...
uint8_t Adafruit_Fingerprint::createModel(void) {
  uint8_t result = 0;
  GET_CMD_PACKET(FINGERPRINT_REGMODEL);
  result = packet.data[0];
  if(result == FINGERPRINT_OK) {
    // Obtener el template del buffer del sensor
    uint8_t uploadResult = getModel();
    if(uploadResult == FINGERPRINT_OK) {
      // El template se recibe en el siguiente paquete de datos
      Adafruit_Fingerprint_Packet templatePacket(FINGERPRINT_DATAPACKET, 256, nullptr);
      if(getStructuredPacket(&templatePacket, DEFAULTTIMEOUT) == FINGERPRINT_OK) {
        memcpy(lastTemplate, templatePacket.data, 256);
        lastTemplateSize = 256;
      }
    }
  }
  return result;
...
/**************************************************************************/
/*! // ELIMINAR DUPLICADOS DESDE AQUÍ
...
Adafruit_Fingerprint::getStructuredPacket(Adafruit_Fingerprint_Packet *packet,
  ...
  while (mySerial->available() < 9) {
    if ((millis() - starttime) > timeout) return FINGERPRINT_TIMEOUT;
  }
  byte = mySerial->read();
  if (byte != (FINGERPRINT_STARTCODE >> 8)) return FINGERPRINT_BADPACKET;
  sum += byte;
  byte = mySerial->read();
  if (byte != (FINGERPRINT_STARTCODE & 0xFF)) return FINGERPRINT_BADPACKET;
  sum += byte;
  uint32_t addr = 0;
  for (int i = 0; i < 4; i++) {
    byte = mySerial->read();
  ...
  packet->type = mySerial->read();
  sum += packet->type;
  packet->length = mySerial->read();
  packet->length <<= 8;
  packet->length |= mySerial->read();
  sum += (packet->length >> 8) & 0xFF;
  sum += packet->length & 0xFF;
  for (uint16_t i = 0; i < packet->length; i++) {
    while (!mySerial->available()) {
      if ((millis() - starttime) > timeout) return FINGERPRINT_TIMEOUT;
    }
    packet->data[i] = mySerial->read();
    sum += packet->data[i];
  }
  uint16_t checksum = mySerial->read();
  checksum <<= 8;
  checksum |= mySerial->read();
  if (sum != checksum) return FINGERPRINT_BADPACKET;
  return FINGERPRINT_OK;
}
...
void Adafruit_Fingerprint::writeStructuredPacket(const Adafruit_Fingerprint_Packet &packet) {
  ...
  sum += FINGERPRINT_STARTCODE >> 8;
  sum += FINGERPRINT_STARTCODE & 0xFF;
  mySerial->write((uint8_t)(theAddress >> 24));
  mySerial->write((uint8_t)(theAddress >> 16));
  mySerial->write((uint8_t)(theAddress >> 8));
  mySerial->write((uint8_t)(theAddress & 0xFF));
  sum += (theAddress >> 24) & 0xFF;
  sum += (theAddress >> 16) & 0xFF;
  sum += (theAddress >> 8) & 0xFF;
  sum += theAddress & 0xFF;
  mySerial->write(packet.type);
  sum += packet.type;
  mySerial->write((uint8_t)(packet.length >> 8));
  mySerial->write((uint8_t)(packet.length & 0xFF));
  sum += packet.length >> 8;
  sum += packet.length & 0xFF;
  for (uint16_t i = 0; i < packet.length; i++) {
