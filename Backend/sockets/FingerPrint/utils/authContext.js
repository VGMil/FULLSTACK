...
// Define el umbral de similitud para la autenticación
const FINGERPRINT_SIMILARITY_THRESHOLD = 0.72; // Umbral para considerar una coincidencia válida
...
async function authenticateUser(fingerprint) {
    ...
    try {
        // Validación básica del formato de huella
        if (!fingerprint || typeof fingerprint !== 'string' || !/^[0-9A-Fa-f]+$/.test(fingerprint)) {
